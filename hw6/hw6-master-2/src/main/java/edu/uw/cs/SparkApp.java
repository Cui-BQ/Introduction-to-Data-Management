 ...
 |-- div5tailnum: integer (nullable = true)
   */
    ...
    public static JavaRDD<Row> QA(SparkSession spark, String dataFile) {
        ...
        // TODO: Your code here       
        d.createOrReplaceTempView("flights");
        Dataset<Row> qA =  spark.sql("SELECT DISTINCT destcityname FROM flights WHERE origincityname = 'Seattle, WA'");
        JavaRDD<Row> rdd = qA.javaRDD();
        return rdd;
    }
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        ...
        // TODO: Your code here        
        Dataset<Row> output = d.filter(d.col("cancelled").lt(1))
                                .groupBy(d.col("origincityname"), d.col("month"))
                                .count();
        return output.javaRDD();
    }
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        ...
        // TODO: Your code here
        Dataset<Row> output = d.filter(d.col("depdelay").isNotNull())
                                .filter(d.col("cancelled").lt(1))
